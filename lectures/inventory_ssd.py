...
def create_sdd_inventory_model(
        ...
        n_z: int = 100,      # Exogenous state discretization size
...
def B2(x, z_idx, v, model):
    ...
    res = B_vec_a(x, z_idx, a_values, v, model)
    return jnp.where(a_values <= max_x - x, res, -jnp.inf)
...
def T(v, model):
    ...
    res = B2_vec_z_x(x_values, z_indices, v, model)
