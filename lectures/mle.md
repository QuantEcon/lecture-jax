...
```{code-cell} ipython3
## TODO: remove this after review or keep it for comparing?
...
jax_dlogL = jax.vmap(jax.grad(jax_logL))

β = jnp.linspace(1, 20)

fig, (ax1, ax2) = plt.subplots(2, sharex=True, figsize=(12, 8))

ax1.plot(β, jax_logL(β), lw=2)
ax2.plot(β, jax_dlogL(β), lw=2)
...
def poisson_logL(β, model):
    ...
    z = μ - jnp.log(jax_factorial(model.y))
    r = model.y * jnp.log(μ)
    return jnp.sum(r - z)
```

```{code-cell} ipython3
G_logL = jax.grad(poisson_logL)
H_logL = jax.jacfwd(G_logL)
```

```{code-cell} ipython3
def newton_raphson_jax(model, β, tol=1e-3, max_iter=100, display=True):
...
# Use newton_raphson to find the MLE
β_hat = newton_raphson_jax(poi,init_β, display=True)
```
